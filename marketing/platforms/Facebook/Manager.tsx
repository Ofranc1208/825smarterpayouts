...
type FacebookManagerProps = { onBack: () => void; };

export default function FacebookManager({ onBack }: FacebookManagerProps) {
  ...
  const styles = {
    ...
    tab: { padding: '0.75rem 1rem', cursor: 'pointer', border: 'none', background: 'transparent', fontSize: '0.95rem', fontWeight: 600 },
    activeTab: { padding: '0.75rem 1rem', cursor: 'pointer', borderTop: 'none', borderLeft: 'none', borderRight: 'none', borderBottom: '2px solid #1d4ed8', background: 'transparent', fontSize: '0.95rem', fontWeight: 600, color: '#1d4ed8' },
    ...
    button: { background: '#1d4ed8', color: '#fff', border: 'none', borderRadius: '10px', padding: '0.75rem 1.5rem', fontWeight: 600, cursor: 'pointer', fontSize: '0.95rem' },
    ...
    selectedTopicChip: { display: 'inline-block', background: '#1d4ed8', color: '#fff', border: '1px solid #1d4ed8', borderRadius: '20px', padding: '0.5rem 1rem', margin: '0.25rem', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 500 },
    textarea: { width: '100%', padding: '0.75rem', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.95rem', minHeight: '120px', resize: 'vertical' as const },
    mediaButtonsRow: { display: 'flex', gap: '0.5rem', marginTop: '0.75rem', marginBottom: '0.75rem' },
    mediaButton: { flex: 1, background: '#1877f2', color: '#fff', border: 'none', borderRadius: '10px', padding: '0.8rem 1rem', fontWeight: 600, cursor: 'pointer', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' },
    ...
    publishButtonEnabled: { display: 'inline-block', background: '#1877f2', color: '#fff', border: 'none', borderRadius: '10px', padding: '0.8rem 1.5rem', fontWeight: 600, cursor: 'pointer', marginTop: '0.75rem', width: '100%', fontSize: '0.95rem' },
    mediaPreview: { width: '100%', maxWidth: '300px', borderRadius: '10px', border: '1px solid #e5e7eb', marginTop: '0.75rem', display: 'block' },
  ...
  const fetchTopics = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/marketing/generate', {
        ...
        body: JSON.stringify({ task: 'generate_topics', platform: 'facebook' }),
  ...
  const generateContent = async (selectedTopic: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/marketing/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ topic: selectedTopic, task: 'social', platform: 'facebook' }),
        signal: controller.signal,
      });
  ...
  const handlePublishToFacebook = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setPublishResult(`✅ Successfully published to Facebook! ${data.url || ''}`);
      } else {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Managing: Facebook</h2>
      ...
      {activeTab === 'generate_content' && (
        <div style={styles.card}>
          ...
          <label htmlFor="topic-facebook" style={{ display: 'block', fontWeight: 600, marginBottom: '0.5rem', color: '#374151' }}>
            Topic:
          </label>
          <input 
            id="topic-facebook" 
            type="text" 
            placeholder="Enter a topic for Facebook" 
            value={topic} 
            ...
            {content && !content.startsWith('Error:') && (
              <div style={{ marginTop: '1rem' }}>
                ...
                <button 
                  ...
                  title={!selectedMedia ? 'Please select an image or video first' : 'Publish to Facebook'}
                >
                  {isPublishing ? 'Publishing…' : selectedMedia ? '📤 Publish to Facebook' : 'Select media to publish'}
                </button>
              </div>
            )}
          </div>
          ...
          {selectedMedia && (
            <div>
              ...
              {selectedMedia.type === 'video' ? (
                <video src={selectedMedia.url} controls muted loop style={styles.mediaPreview} title="Selected Facebook video">
                  Your browser does not support the video tag.
                </video>
              ) : (
                <img src={selectedMedia.url} alt="Selected Facebook image" style={styles.mediaPreview} />
              )}
            </div>
          )}
          {publishResult && (
